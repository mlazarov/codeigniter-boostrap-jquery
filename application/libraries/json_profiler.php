...
/**
 * CodeIgniter JSON Profiler Class
 *
 * This class enables you to display benchmark, query, and other data as json
...
class JSON_Profiler {
	...
	protected function _compile_queries() {
		...
		foreach ($dbs as $db) {
			...
			if (count($db->queries) == 0) {
				$db_queries[$db->database] = 'No queries';
				...
				foreach ($db->queries as $key => $val) {
					...
					$db_queries[$db->database][] = array('query'=>$val,$time=>$time);
	...
	protected function _compile_http_headers() {
		...
		$output .= '<legend style="color:#000;">&nbsp;&nbsp;'.$this->CI->lang->line('profiler_headers').
				'&nbsp;&nbsp;(<span style="cursor: pointer;" onclick="var s=document.getElementById(\'ci_profiler_httpheaders_table\').style;s.display=s.display==\'none\'?\'\':\'none\';'.
				'this.innerHTML=this.innerHTML==\''.$this->CI->lang->line('profiler_section_show').'\'?\''.$this->CI->lang->line('profiler_section_hide').'\':\''.
				$this->CI->lang->line('profiler_section_show').'\';">'.$this->CI->lang->line('profiler_section_show').'</span>)</legend>';
		$output .= "\n";

		$output .= "\n\n<table style='width:100%;display:none' id='ci_profiler_httpheaders_table'>\n";

		foreach (array('HTTP_ACCEPT', 'HTTP_USER_AGENT', 'HTTP_CONNECTION', 'SERVER_PORT', 'SERVER_NAME', 'REMOTE_ADDR', 
						'SERVER_SOFTWARE', 'HTTP_ACCEPT_LANGUAGE', 'SCRIPT_NAME', 'REQUEST_METHOD',' HTTP_HOST', 'REMOTE_HOST', 
						'CONTENT_TYPE', 'SERVER_PROTOCOL', 'QUERY_STRING', 'HTTP_ACCEPT_ENCODING', 'HTTP_X_FORWARDED_FOR') as $header) {
			$val = (isset($_SERVER[$header])) ? $_SERVER[$header] : '';
			
			$headers[$header] = $val;
			
			$output .= "<tr><td style='vertical-align: top;width:50%;padding:5px;color:#900;background-color:#ddd;'>".$header.
			"&nbsp;&nbsp;</td><td style='width:50%;padding:5px;color:#000;background-color:#ddd;'>".$val."</td></tr>\n";
		}
	...
	protected function _compile_config(){
		...
		foreach ($this->CI->config->config as $config=>$val) {
			...
			$output .= "<tr><td style='padding:5px; vertical-align: top;color:#900;background-color:#ddd;'>".$config.
	...
	public function run() {
		...
		foreach ($this->_available_sections as $section) {
			if ($this->_compile_{$section} !== FALSE) {
				...
				$output[$section]= $this->{$func}();
				$fields_displayed++;
			}
		}

		if ($fields_displayed == 0) {
			$output['profiler_info'] = $this->CI->lang->line('profiler_no_profiles');
		}
...
/* Location: ./libraries/Profiler.php */
